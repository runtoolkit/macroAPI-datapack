...
$execute unless data storage macro:engine tools_trigger.config{silent:1} run tellraw @a[tag=macro.debug] ["",{"text":"[macroAPI] ","color":"#00AAAA","bold":true},{"text":"tools/trigger ","color":"aqua"},{"text":"► ","color":"yellow"},{"text":"$(type)","color":"white"}]
