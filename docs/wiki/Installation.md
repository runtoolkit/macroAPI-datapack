...
```bash
# Windows
certutil -hashfile macroAPI-datapack.zip SHA256

# Linux / macOS
sha256sum macroAPI-datapack.zip
```
...
└── world/
    └── datapacks/
        └── macroAPI-datapack/   ← extract ZIP here (or place ZIP directly)
```
...
If your server uses other LanternLoad-compatible datapacks, macroAPI integrates automatically. Version detection:

```mcfunction
# Check if macroAPI is loaded
execute if score macroAPI load.status matches 1.. run say loaded

# Get version (major*10000 + minor*100 + patch)
# v4.0.1 = 40000
scoreboard players get macroAPI load.status
```
...
To fully disable macroAPI and clean all scoreboards/storage:
