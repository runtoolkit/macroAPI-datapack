...
On `/reload` or world startup, macroAPI fires a confirmation gate **before** initializing storage. This prevents overwriting existing session data (permissions, cooldowns, wand binds, etc.).

```
# In server log / chat:
[macroAPI] Reload detected. Run /function ame_load:load/yes to confirm.
...
| `macro:disable` | Disables macroAPI | `ame_load:gate/yes` / `gate/no` |
...
macroAPI's tick loop is split into five independently-togglable subsystems:
