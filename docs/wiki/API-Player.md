...
macroAPI assigns each player a unique integer PID stored in `macro:engine player_pids`. PID-based selectors (`scores={macro.pid=N}`) are used internally for reliable targeting even on servers where multiple players might share the same name (offline mode).
